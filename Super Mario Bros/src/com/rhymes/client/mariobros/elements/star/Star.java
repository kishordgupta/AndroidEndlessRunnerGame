package com.rhymes.client.mariobros.elements.star;
...
public class Star extends Element implements InteractionCollisionCallbacks {
	...
	private void initAnimations() {
		...
		Star.statimagemove = new Starimagemove(this);
		Star.statimagemove.init();
	}
	...
	public void animate(long elapsedTime) {
		...
		if (count < 1) {
			if (state != STATE_DESTROYED) {
				...
				this.setmBitmap(Starimagemove.bitmap[r]);
			} else
				this.setmBitmap(Starimagemove.bitmap1);
			count = 5;
		}
		count--;
		Star.statimagemove.play(elapsedTime);
		if (!died) {
	...
	public void onCollision(Collision collision) {
		...
		if (otherElement instanceof icopter) {
			...
			Icoptermove.point=Icoptermove.point+5;
