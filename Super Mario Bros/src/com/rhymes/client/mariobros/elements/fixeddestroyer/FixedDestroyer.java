package com.rhymes.client.mariobros.elements.fixeddestroyer;
...
public class FixedDestroyer extends Element implements InteractionCollisionCallbacks {

	// private static Bitmap mBitmap = BitmapFactory.decodeResource(
	// Frame.resources, R.drawable.cloud_triple);

	private static FixedDestroyerimagemove statimagemove;
	private final int STATE_NORMAL = 0;
	private final int STATE_DESTROYED = 2;

	private int state = STATE_NORMAL;

	public FixedDestroyer(int x, int y, int width, int height) {
	...
	private void initAnimations() {
		...
		FixedDestroyer.statimagemove = new FixedDestroyerimagemove(this);
		FixedDestroyer.statimagemove.init();
	}
...
//			} else
		if (state != STATE_DESTROYED) {
				this.setmBitmap(FixedDestroyerimagemove.bitmap1);}
		else
			this.setmBitmap(FixedDestroyerimagemove.bitmap);
//			count = 5;
//		}
//		count--;
	...
	public void onCollision(Collision collision) {
		...
		if (collision.element1 instanceof FixedDestroyer) {
